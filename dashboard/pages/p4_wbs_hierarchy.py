...
def render():
    ...
    <div class="metric-card" style="border:1px solid #2E86AB; margin-bottom:16px;">
        ...
        <p style="color:#8899AA; font-size:13px; margin:4px 0 0;">
            ...
            <b>Rollup logic:</b> Progress = weighted average by estimated hours; Status = worst child propagates up.
    ...
    def render_node(item, depth=0):
        ...
        if item.get("type") == "group":
            ...
            total_est = sum(t.get("estimated_hours", 0) for t in tasks_in_group)
            ...
            st.markdown(f"""
            {indent} <span style="color:#F6AE2D; font-weight:600;">▼ {name}</span>
            <span style="color:#8899AA;">({avg_progress:.0f}% complete) {emoji}</span>
            <br>{indent}&nbsp;&nbsp;&nbsp;
            <span style="color:#556677; font-size:12px;">Est: {total_est}h | Act: {total_act:.1f}h | Tasks: {len(tasks_in_group)}</span>
            """, unsafe_allow_html=True)
            ...
            <span style="color:#8899AA; font-size:12px;"> — {pct}% | Est:{est}h Act:{act:.1f}h | {res_str}</span>
            """, unsafe_allow_html=True)
    ...
    total_est = sum(t.get("estimated_hours", 0) for t in all_tasks)
    ...
    <span style="color:#8899AA;">Est: {total_est}h | Act: {total_act:.1f}h | Teams: {len(data)} | Tasks: {len(all_tasks)}</span>
    """, unsafe_allow_html=True)
    st.divider()
    ...
    <div class="metric-card" style="border:1px solid #556677;">
        ...
        <ul style="color:#8899AA; font-size:13px;">
            <li><b>Hours:</b> Sum all child tasks</li>
            <li><b>Progress:</b> Weighted average by estimated hours. Formula: Σ(child_progress × child_est_hours) / Σ(child_est_hours). Falls back to simple average when estimates are 0.</li>
